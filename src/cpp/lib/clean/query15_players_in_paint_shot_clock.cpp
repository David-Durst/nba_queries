...
#pragma omp parallel for
    for (int64_t time = 0; time < moments->size; time++) {
        auto start_outer = Halide::Tools::benchmark_now();
        int thread_num = omp_get_thread_num();
        if (moments->shot_clock[time] < end_time) {
            auto start_outer_if = Halide::Tools::benchmark_now();
            for (int i = 0; i < NUM_PLAYERS_AND_BALL; i++) {
                auto start_inner_for = Halide::Tools::benchmark_now();
                if (point_intersect_no_time(&paint0, moments->x_loc[i][time], moments->y_loc[i][time]) ||
                    ...
                    auto start_inner_if = Halide::Tools::benchmark_now();
                    temp_players[thread_num].push_back({time, moments->game_clock[time], moments->player_id[i][time]});
                    temp_time[3 * num_threads + thread_num] += Halide::Tools::benchmark_duration_seconds(start_inner_if, Halide::Tools::benchmark_now());
                }
                temp_time[2 * num_threads + thread_num] += Halide::Tools::benchmark_duration_seconds(start_inner_for, Halide::Tools::benchmark_now());
            }
            temp_time[1 * num_threads + thread_num] += Halide::Tools::benchmark_duration_seconds(start_outer_if, Halide::Tools::benchmark_now());
        }
        temp_time[0 * num_threads + thread_num] += Halide::Tools::benchmark_duration_seconds(start_outer, Halide::Tools::benchmark_now());
    }
...
#pragma omp parallel for
    for (int64_t time = 0; time < moments->size; time++) {
        ...
        if (moments->shot_clock[time] < end_time) {
            for (int i = 0; i < NUM_PLAYERS_AND_BALL; i++) {
                if ((moments->x_loc[i][time] >= paint0.start.x && moments->x_loc[i][time] < paint0.start.x &&
                     moments->y_loc[i][time] >= paint0.start.y && moments->y_loc[i][time] < paint0.start.y) ||
                    (moments->x_loc[i][time] >= paint1.start.x && moments->x_loc[i][time] < paint1.start.x &&
                     moments->y_loc[i][time] >= paint1.start.y && moments->y_loc[i][time] < paint1.start.y)) {
                    temp_players[thread_num].push_back({time, moments->game_clock[time], moments->player_id[i][time]});
                }
            }
        }
